...
Текущая версия: **v1.4.0** ([история изменений](CHANGELOG.md))
...
- Кнопка «🔍 Найти игру» прямо в Telegram-боте — запускает поиск матча удалённо, не подходя к компьютеру (Dota 2 должна быть уже открыта).
- Кнопка «🧭 Роли» в Telegram-боте — меняет роли ролевого поиска (можно выбрать несколько) прямо на экране Dota 2, независимо от поиска игры.
- Подтверждение принятия прямо в Telegram (по желанию, выключено по умолчанию) — вместо автопринятия бот присылает «Принять»/«Отклонить» и ждёт ответа.
- Автопринятие приглашений в пати (по желанию, выключено по умолчанию) — не пропустишь заявку от друзей, даже если отошёл от компьютера.
...
> **Windows покажет «Windows защитила ваш ПК» при первом запуске.** Это не угроза безопасности — просто `.exe` пока не подписан сертификатом и не успел накопить репутацию в базе Microsoft (она появляется только после многих запусков файла разными людьми). Нажмите «Подробнее» → «Выполнить в любом случае». Исходный код полностью открыт в этом репозитории — можно проверить самостоятельно или собрать `.exe` из исходников.
...
python -m PyInstaller --onefile --windowed --name DotaNotifier --icon app_icon.ico ^
  ...
  --add-data "role_offlane_on_ru.png;." --add-data "role_offlane_off_ru.png;." ^
  --add-data "role_support_on_ru.png;." --add-data "role_support_off_ru.png;." ^
  --add-data "role_hard_support_on_ru.png;." --add-data "role_hard_support_off_ru.png;." ^
